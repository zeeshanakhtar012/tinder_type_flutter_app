...
class ItemEducationBottomSheet {
  static void show(BuildContext context, User user) {
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        selectedOption.value= user.reference?.education ?? '';
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  GradientText(
                      text: "Education Level", style: AppFonts.subscriptionBlaxityGold).paddingSymmetric(
                    vertical: 6.h,
                  ),

                  Wrap(children: List.generate(
                      controllerRegistration.educationLevels.length,
                          (value) {
                        return Obx(() {
                          return IntrinsicWidth(
                            child: buildSelectOneOption(
                                controllerRegistration.educationLevels[value],
                                    () {
                      ...
                      onTap: () async {
                        if (selectedOption.value.isNotEmpty) {
                          await controllerRegistration.updateCoupleProfile(
                              user, education: selectedOption.value);
                          Get.back();
                        }
                        else {
                          FirebaseUtils.showError("Please select your Education.");
                        }
                      },
